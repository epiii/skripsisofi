...
// switch($_GET['act']){
  ...
  case "detailsewa":
    $edit = mysqli_query($con,"SELECT * FROM sewas,kustomer WHERE sewas.id_kustomer=kustomer.id_kustomer AND id_sewas='$_GET[id]'");
    $r    = mysqli_fetch_array($edit);
    $tanggal=tgl_indo($r['tgl_sewa']);
    
    if ($r['status_sewa']=='Baru'){
        $pilihan_status = array('Baru', 'DP', 'Lunas');
    }
    elseif ($r['status_sewa']=='DP'){
        $pilihan_status = array('DP','Lunas', 'Batal');    
    }
    elseif ($r['status_sewa']=='Lunas'){
    ...
    foreach ($pilihan_status as $status) {
	   $pilihan_sewa .= "<option value=$status";
	   if ($status == $r['status_sewa']) {
		    $pilihan_sewa .= " selected";
	   }
	   $pilihan_sewa .= ">$status</option>\r\n";
    }
    ...
    echo "
                ...
                <section class='content invoice'>
                    ...
                    <div class='row invoice-info'>
                        ...
                        <div class='col-sm-4 invoice-col'>
                        <form method=POST action=$aksi?module=sewa&act=update>
          <input type=hidden name=id value=$r[id_sewas]>
                            <b>Invoice #$r[id_sewas]</b><br/>
                            <b>Status sewa : </b><select name=status_sewa onChange='this.form.submit()'> $pilihan_sewa</select><br/>
                            </form>
                        </div><!-- /.col -->
                    </div><!-- /.row -->";
                     // tampilkan rincian produk yang di sewa
  $sql2=mysqli_query($con,"SELECT * FROM sewas_detail, produk 
                     WHERE sewas_detail.id_produk=produk.id_produk 
                     AND sewas_detail.id_sewas='$_GET[id]'");
  echo " <!-- Table row -->
                    <div class='row'>
   ...
   echo "
                    ...
                    <div class='row'>
                        ...
                        <div class='col-xs-6'>
                            <p class='lead'>Payment Methods:</p>";
                             $bank=mysqli_query($con,"SELECT * FROM mod_bank sewa BY id_bank ASC");
      	...
      	echo "
                          ...
                          echo "
                            ...
                            Pembayaran dapat dilakukan melalui transfer bank ke Rekening kami :<br />";
                             $norek=mysqli_query($con,"SELECT * FROM mod_bank sewa BY id_bank ASC");
      while($bb=mysqli_fetch_array($norek)){
      	echo "- <b>$bb[nama_bank] No. Rek : $bb[no_rekening] A.n. $bb[pemilik]</b><br />";
                        ...
                        <div class='col-xs-6'>";
                         $cekpembeli=mysqli_fetch_array(mysqli_query($con,"SELECT * FROM kustomer,sewas 
          WHERE sewas.id_kustomer=kustomer.id_kustomer AND id_sewas='$_GET[id]'"));
